...
async function initIndexPage() {
    let currentSeries = 'vespa', currentJenis = 'ALL', searchTerm = '';
    ...
    function renderProductCards(container, productList) {
        ...
        productList.forEach((barang, index) => {
            ...
            el.innerHTML = `<img src="${barang.foto}" alt="${barang.nama}" class="produk-img"/><h3>${barang.nama}</h3><p class="produk-desc">${barang.deskripsi||""}</p><div class="produk-specs"><p><strong>Bahan:</strong> ${barang.bahan||"N/A"}</p><p><strong>Suara:</strong> ${barang.suara||"N/A"}</p><p><strong>Mesin:</strong> ${barang.rekomendasi_mesin||"N/A"}</p></div><p class="produk-stok">${barang.stok||""}</p><p class="produk-harga">Rp${barang.harga.toLocaleString("id-ID")}</p><div class="produk-actions"><button class="produk-btn" onclick="bukaModalPilihJenis('${barang._id}')">Tambah ke Keranjang</button><button class="produk-btn-secondary" onclick="bukaModalMarketplace('${barang._id}')"><i class="fas fa-store"></i> Marketplace</button></div>`;
    ...
    function renderJenisButtons() {
        ...
        kategoriMotor[currentSeries].jenis.forEach(jenis => {
            ...
            btn.textContent = jenis; btn.dataset.jenis = jenis;
